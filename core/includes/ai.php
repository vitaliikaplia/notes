...
7. Content uses Markdown (headings, lists, checklists, code, quotes). Images: ![caption](url). A grid of thumbnails (gallery block): a line `::: gallery cols=3` (cols 2-4), then one ![caption](url) per line, closed with a line `:::`. A video file URL in the same tag (![caption](/file/….mp4)) renders as a video player.
...
function ai_tool_notes_update(array $args): array {
    ...
    // Uploads dropped by this update are orphans now — remove them like the editor does
    if($markdown !== null) {
        delete_orphaned_uploads(
            extract_image_urls($existing['content']['blocks'] ?? []),
            extract_image_urls($blocks)
        );
    }
...
function ai_tool_notes_delete(array $args): array {
    ...
    $note_uploads = collect_note_image_urls($relative);
    if(!delete_note($relative)) {
        throw new RuntimeException('Failed to delete note');
    }
    foreach($note_uploads as $url) {
